...
/*
    ...
    Autor: Julio Tuozzo
    Funci�n: Formulario de listado de �reas.
...
if (isset($_POST[filtrar]))
    ...
    $_SESSION[EQUIPO_P][id_equipo_princ]=$_POST[id_equipo_princ];
    $_SESSION[EQUIPO_P][nombre]=$_POST[nombre];
    $_SESSION[EQUIPO_P][activo]=$_POST[activo];
    }

if (isset($_SESSION[AREA]))
   {$condicion="";
    if (strlen($_SESSION[AREA][area_id])>0)
        {$condicion.="id_equipo_princ like '%{$_SESSION[AREA][area_id]}%' AND ";
        }
...
if ($_GET[pagina]<1)
    ...
    $query="SELECT COUNT(*) AS cuantos
            FROM {$MyPHD}equipo_principal $condicion";
    $result=mysql_query($query) or die (mysql_error());
    $row = mysql_fetch_array($result);
    $q_registros=$row['cuantos'];
    $pagina=1;
    $orden="id_equipo_princ";
    $sentido="ASC";
...
$desde_reg=($pagina-1)*$_SESSION[PHD_MAX_LINES_SCREEN];
    ...
    $query="SELECT * FROM {$MyPHD}equipo_principal $condicion ORDER BY $orden $sentido LIMIT $desde_reg, $_SESSION[PHD_MAX_LINES_SCREEN]";
...
## Formulario de listado de �reas.
// Area list form.

require($Include.'equipo_list.inc');
?>
